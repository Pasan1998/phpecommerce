    ...
    <body class="text-center">
        <main class="form-signin w-100 m-auto border border-3 border-info">
            ...
            if ($_SERVER['REQUEST_METHOD'] == "POST") {
                extract($_POST);
                $resettokennumber = cleanInput($resettokennumber);
                ...
                if (!empty($resettokennumber)) {
                    echo $sql = "SELECT * FROM tbl_users WHERE passwordreset= '$resettokennumber' ";
                    $db = dbConn();
                ...
                if (empty($messages)) {
                    ...
                    $sql = "UPDATE  tbl_users SET Password= '$password' WHERE UserId= '$resetuseridpassword2' ";
                    $db = dbConn();
                    $results = $db->query($sql);
            ...
            <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
                <img class="mb-2 logoimage" src="<?= SYSTEM_PATH ?>" alt="logo" >
